#ifndef __SERIAL_SEND_H_
#define __SERIAL_SEND_H_
#include <Windows.h>
#include <string>
#include "stdint.h"
using namespace std;
...
public:
    //同步方式打开串口,并配置默认信息
    bool openSyn(string serial_name,DWORD baud_rate,unsigned char parity, unsigned char byte_size, unsigned char stop_bits);
    //同步方式打开串口(需自己配置串口参数)
    ...
    bool flushBuff();
    //写数据
    DWORD writeData(uint8_t *buffer,int length);
    //读数据
    DWORD readData(uint8_t *buffer,int length);
    //写字符串
